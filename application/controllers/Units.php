...
class Units extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['title']              = 'Apotik Medika - List Kemasan';
        $data['breadcrumb_title']   = 'List Kemasan';
        $data['breadcrumb_path']    = 'Manajemen Barang / List Kemasan';
    ...
    public function search($page = null)
    {
        ...
        $data['title']              = 'Apotik Medika - Cari Staff';
    ...
    public function edit($id)
    {
        ...
        if (!$this->units->validate()) {
            $data['title']              = 'Apotik Medika - Edit Kemasan Barang';
            $data['page']               = 'pages/units/edit';
            $data['breadcrumb_title']   = 'Edit Kemasan Barang';
            $data['breadcrumb_path']    = "Manajemen Barang / Edit Kemasan Barang / " . $data['input']->nama;

            return $this->view($data);
        }

        if ($this->units->where('id', $id)->update($data['input'])) {   // Update data
            $this->session->set_flashdata('success', 'Data kemasan berhasil diubah');
        } else {
    ...
    {
        if($this->db->where('id', $id)->delete('satuan')){
            $this->session->set_flashdata('success', 'Kemasan Berhasil Dihapus');
        }
        else {
        $this->session->set_flashdata('error', 'Oops, terjadi suatu kesalahan');
        }
