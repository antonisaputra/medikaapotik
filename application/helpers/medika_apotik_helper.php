...
// Mendapatkan seluruh list kategori obat
...
// Mendapatan kategori obat berdasarkan id
function getKategoriName($id_kategori)
{
    $CI = &get_instance();
...
function getItemsName($id_barang)
{
    $CI = &get_instance();
    $CI->db->where('id_barang', $id_barang);
    return $CI->db->get('barang')->row()->nama;
}
...
function getTodayEarning()
{
    ...
    $CI->db->where('DATE(waktu_pengeluaran)', date('Y-m-d'));
...
/**
 * Mendapatkan banyak yang disediakan
 */
 ...
 * tipe barang  dari tabel detail_penjualan
 */
